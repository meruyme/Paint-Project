...
/**
 * JDialog que será usado para o usuário editar as informações referentes a um retângulo. 
 * @author Mellany Linhares
 *
 */
public class EditarRetangulo extends JDialog {
	 ...
	 */
	public EditarRetangulo(Formas formas, PaintJPanel panel,DefaultListModel modelo, int formaSelecionada) {
			    
	    Retangulo rect = (Retangulo) formas.getFormas().get(formaSelecionada);
	    corSelecionada = rect.getCor();
	    
		setModalityType(DEFAULT_MODALITY_TYPE);
		setTitle("Editar retângulo");
		...
		txtBase.setText(Integer.toString(rect.getBase()));
		...
		btnSelecionarCor.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				corSelecionada = JColorChooser.showDialog(EditarRetangulo.this,"Selecione uma cor", corSelecionada );
				mostrarCor.setOpaque(true);
				mostrarCor.setBackground(corSelecionada);
			}
		});
		...
		txtY.setText(Integer.toString(rect.getPosicaoY()));
		...
		txtX.setText(Integer.toString(rect.getPosicaoX()));
		contentPanel.add(txtX);
		
		txtAltura = new JTextField();
		txtAltura.setColumns(10);
		txtAltura.setBounds(159, 97, 150, 22);
		txtAltura.setText(Integer.toString(rect.getAltura()));
		...
		
		{
			...
			getContentPane().add(buttonPane, BorderLayout.SOUTH);
			{
				 ...
				 * Foi utilizada uma variável de controle para saber se a forma foi editada ou não. A variável de controle recebe o return do método editarRetângulo.
				...
				btnDesenhar.addMouseListener(new MouseAdapter() {
					...
					public void mouseClicked(MouseEvent e) {
						...
						contrl = rect.editarRetangulo(txtX.getText(), txtY.getText(), corSelecionada, preenchido, txtBase.getText(), txtAltura.getText(),formas,formaSelecionada);
						if(contrl == true) {
							formas.getFormas().set(formaSelecionada, rect);
							modelo.set(formaSelecionada, rect.toString());
							panel.repaint();
							dispose();
						}
					}
